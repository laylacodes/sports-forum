...
module("Integration | Component | d-modal", function(hooks) {
    ...
    `, {
        ...
        assert.dom(".d-modal .modal-close").doesNotExist("close button is not shown when dismissable=false");
        testState.dismissable = true;
        await settled();
        assert.dom(".d-modal .modal-close").exists("close button is visible when dismissable=true");
        await click(".d-modal .modal-close");
        assert.true(testState.closeModalCalled, "closeModal is called when close button clicked");
    });
    ...
    test("as a form", async function(assert) {
        ...
        const handleSubmit = (event1)=>{
            ...
            submittedFormData = new FormData(event1.currentTarget);
        };
        await render(template(`
