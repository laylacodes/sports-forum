...
module("Integration | Component | ComposerEditor", function(hooks) {
    ...
    test("warns about users that will not see a mention", async function(assert) {
        const model = {};
        const noop = ()=>{};
        const expectation = (warning1)=>{
            if (warning1.name === "user-no") {
                assert.deepEqual(warning1, {
                    name: "user-no",
                    reason: "a reason"
                });
            } else if (warning1.name === "user-nope") {
                assert.deepEqual(warning1, {
                    name: "user-nope",
                    reason: "a reason"
                });
    ...
    `, {
        ...
        assert.strictEqual(query(".d-editor-preview").innerHTML.trim(), '<p>"&gt;<svg></svg></p>');
    });
});
