...
function findItem(target, method, collection) {
    var index = -1;
    for (var i = 0, l = collection.length; i < l; i += QUEUE_ITEM_LENGTH) {
...
function findTimerItem(target, method, collection) {
    var index = -1;
    for (var i = 2, l = collection.length; i < l; i += TIMERS_OFFSET) {
        if (collection[i] === target && collection[i + 1] === method) {
            index = i - 2;
            break;
        }
...
Queue.prototype.stackFor = function stackFor (index) {
    if (index < this._queue.length) {
        var entry = this._queue[(index * QUEUE_ITEM_LENGTH) + 3];
        if (entry) {
            return entry.stack;
        }
        else {
...
Queue.prototype.consoleTaskFor = function consoleTaskFor (index, inQueueBeingFlushed) {
        if ( inQueueBeingFlushed === void 0 ) inQueueBeingFlushed = false;

    var q = inQueueBeingFlushed ? this._queueBeingFlushed : this._queue;
    if (index < q.length) {
...
Queue.prototype.push = function push (target, method, args, stack, consoleTask) {
...
Queue.prototype.pushUnique = function pushUnique (target, method, args, stack, consoleTask) {
    ...
    if (index === undefined) {
        var queueIndex = this._queue.push(target, method, args, stack, consoleTask) - QUEUE_ITEM_LENGTH;
 ...
 * @return queue
 */
DeferredActionQueues.prototype.schedule = function schedule (queueName, target, method, args, onceFlag, stack, consoleTask) {
...
var prototypeAccessors = { counters: { configurable: true },defaultQueue: { configurable: true } };
  ...
  */
  ...
  Backburner.prototype.schedule = function schedule (queueName) {
      ...
      var consoleTask = this.createTask(queueName, method);
  ...
  */
  Backburner.prototype.scheduleIterable = function scheduleIterable (queueName, iterable) {
      ...
      var stack = this.DEBUG ? new Error() : undefined;
      var consoleTask = this.createTask(queueName, null);
  ...
  Backburner.prototype.debounce = function debounce () {
      ...
      if (index === -1) {
          ...
          if ((index + TIMERS_OFFSET) === i) {
              ...
              var consoleTask = this._timers[index + 6];
  ...
  Backburner.prototype._later = function _later (target, method, args, wait) {
      var stack = this.DEBUG ? new Error() : undefined;
      var consoleTask = this.createTask('(timer)', method);
      var executeAt = this._platform.now() + wait;
      var id = UUID++;
  ...
  Backburner.prototype.createTask = function createTask (queueName, method) {
      if (this.ASYNC_STACKS && console['createTask']) {
          return console.createTask(("runloop " + queueName + " | " + ((method === null || method === void 0 ? void 0 : method.name) || '<anonymous>')));
      }
  };
