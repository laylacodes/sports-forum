(function (global, factory) {
  ...
  function parseNewTree(transaction){const{state,input,config:options}=transaction;const{measure}=state;if(typeof input==="string"){measure("parsing input for new tree");const{childNodes}=Internals.parse(input,options);const vTree=createTree(childNodes);if(vTree){transaction.newTree=vTree;}measure("parsing input for new tree");}}

  const rawElementsDefaults=["script","noscript","style","template"];const voidElementsDefaults=["area","base","br","col","embed","hr","img","input","link","meta","param","source","track","wbr"];const endTagOmissionRules={li:{li:EMPTY.NUM},dt:{dt:EMPTY.NUM,dd:EMPTY.NUM},dd:{dt:EMPTY.NUM,dd:EMPTY.NUM},td:{td:EMPTY.NUM,th:EMPTY.NUM},th:{td:EMPTY.NUM,th:EMPTY.NUM},tbody:{tbody:EMPTY.NUM,tfoot:EMPTY.NUM},tfoot:{tbody:EMPTY.NUM,tfoot:EMPTY.NUM}};const openComment=/<!--/g;const closeComment=/-->/g;const openTag=/<([^\s\\/>]*)\s?/g;const closeTag=/\s?(\/>)|(<\/(.*?)>)/g;const attribute=/([^>\/\n ])*?(["'])(?:(?=(\\?))\3.)*?\2|.*?(?=\/>|>|\n| )/gsm;const parseAttr=/([^=]*)=(.*)|([^>])/gsm;const text=/([^<]*)/g;function removeQuotes(value){if(typeof value!=="string"){return value}const quotes=["\"","'"];const rootQuote=quotes.indexOf(value[0]);const hasRootQuote=rootQuote!==-1;const trailingQuote=hasRootQuote&&quotes.indexOf(value[value.length-1]);if(rootQuote!==-1&&trailingQuote===rootQuote){return value.slice(1,value.length-1)}return value}function parse(html,options={}){const root=createTree("#document-fragment",null,[]);if(!html){root.childNodes.push(createTree("#text",EMPTY.STR));return root}if(!options.parser){options.parser=EMPTY.OBJ;}const blockText=new Set(getConfig("rawElements",rawElementsDefaults,"array",options.parser));const voidElements=new Set(getConfig("voidElements",voidElementsDefaults,"array",options.parser));if(!html.includes("<")&&!html.includes(">")){const newTree=createTree("#text",html);root.childNodes.push(newTree);return root}const stack=[root];let i=0;let pointer=root;let isOpen=false;let lastCommentIndex=html.indexOf("<!--");const resetPointer=()=>{const newTree=createTree(stack.pop());pointer=stack[stack.length-1];pointer.childNodes.push(newTree);};while(i<html.length){openComment.lastIndex=i;closeComment.lastIndex=i;openTag.lastIndex=i;closeTag.lastIndex=i;attribute.lastIndex=i;text.lastIndex=i;parseAttr.lastIndex=0;const shouldSeekComment=Boolean(lastCommentIndex!==null&&lastCommentIndex<=i);const{index:openCommentIndex}=shouldSeekComment&&openComment.exec(html)||EMPTY.OBJ;if(shouldSeekComment){lastCommentIndex=openCommentIndex;}if(openCommentIndex===-1){lastCommentIndex=null;}const isNotRoot=pointer!==root;const isBlockElement=pointer&&blockText.has(pointer.nodeName);if(openCommentIndex===i){let{index:closeCommentIndex}=closeComment.exec(html)||EMPTY.OBJ;if(closeCommentIndex===-1){closeCommentIndex=html.length;}const comment=createTree("#comment");comment.nodeValue=html.slice(i+4,closeCommentIndex);pointer.childNodes.push(comment);i=closeCommentIndex+3;continue}const{0:fullOpenTagMatch,1:tagName,index:openTagIndex}=openTag.exec(html)||EMPTY.OBJ;if(openTagIndex===i&&tagName&&!isBlockElement){if(tagName[0]==="!"){i=html.indexOf(">",openTagIndex)+1;continue}const newTree={rawNodeName:tagName,nodeName:tagName,childNodes:[],attributes:{},nodeType:EMPTY.NUM,nodeValue:EMPTY.STR,key:EMPTY.STR};const supportsEndTagOmission=endTagOmissionRules[tagName];if(supportsEndTagOmission&&supportsEndTagOmission[pointer.nodeName]){resetPointer();}pointer=newTree;stack.push(pointer);isOpen=true;i=openTagIndex+fullOpenTagMatch.length;continue}const{0:fullAttributeMatch,index:attributeIndex}=attribute.exec(html)||EMPTY.OBJ;const attributeMatchTrim=attributeIndex===i&&fullAttributeMatch.trim();if(isOpen&&attributeIndex===i){if(!attributeMatchTrim){i=i+fullAttributeMatch.length+1;if(html[i-1]===">"){const isEnd=i===html.length;if(html[i-2]==="/"||voidElements.has(pointer.nodeName)||isEnd){resetPointer();}isOpen=false;}continue}const{1:key=fullAttributeMatch,2:value=fullAttributeMatch}=parseAttr.exec(attributeMatchTrim)||EMPTY.OBJ;const isBoolean=key===value||value===undefined;const trimKey=key.trim();pointer.attributes[trimKey]=isBoolean?Boolean(value):removeQuotes(value);i=attributeIndex+fullAttributeMatch.length;continue}if(isBlockElement){const closeTag=`</${pointer.nodeName}>`;let closeTagIndex=html.indexOf(closeTag,i);if(closeTagIndex===-1){closeTagIndex=html.length;}const innerText=html.slice(i,closeTagIndex);if(innerText){pointer.childNodes.push(createTree("#text",innerText));}i=closeTagIndex+closeTag.length;isOpen=false;resetPointer();continue}if(html[i]===">"){isOpen=false;i=i+1;if(voidElements.has(pointer.nodeName)){resetPointer();}continue}const{0:fullCloseTagMatch,3:closeTagName,index:closeTagIndex}=closeTag.exec(html)||EMPTY.OBJ;if(closeTagIndex===i&&fullCloseTagMatch){const isVoidElement=voidElements.has(closeTagName);if(fullCloseTagMatch[1]==="/"&&isNotRoot&&!isVoidElement){resetPointer();}isOpen=false;i=closeTagIndex+fullCloseTagMatch.length;continue}const{0:fullTextMatch,index:textIndex}=text.exec(html)||EMPTY.OBJ;if(!isOpen&&textIndex===i&&fullTextMatch.length){const newTree=createTree("#text",fullTextMatch);pointer.childNodes.push(newTree);i=textIndex+fullTextMatch.length;if(i===html.length&&isNotRoot){resetPointer();}continue}pointer.childNodes.push(createTree("#text",html.slice(i,html.length)));i=html.length;}if(root.childNodes.length&&root.childNodes[0].nodeName==="html"){const head={before:[],after:[]};const body={after:[]};const HTML=root.childNodes[0];let beforeHead=true;let beforeBody=true;HTML.childNodes=HTML.childNodes.filter(el=>{if(el.nodeName==="body"||el.nodeName==="head"){if(el.nodeName==="head")beforeHead=false;if(el.nodeName==="body")beforeBody=false;return true}else if(el.nodeType===NODE_TYPE.ELEMENT){if(beforeHead&&beforeBody)head.before.push(el);else if(!beforeHead&&beforeBody)head.after.push(el);else if(!beforeBody)body.after.push(el);}});if(!HTML.childNodes[0]||HTML.childNodes[0].nodeName!=="head"){const headInstance=createTree("head",null,[]);if(headInstance){const existing=headInstance.childNodes;existing.unshift.apply(existing,head.before);existing.push.apply(existing,head.after);HTML.childNodes.unshift(headInstance);}}else {const existing=HTML.childNodes[0].childNodes;existing.unshift.apply(existing,head.before);existing.push.apply(existing,head.after);}if(!HTML.childNodes[1]||HTML.childNodes[1].nodeName!=="body"){const bodyInstance=createTree("body",null,[]);if(bodyInstance){const existing=bodyInstance.childNodes;existing.push.apply(existing,body.after);HTML.childNodes.push(bodyInstance);}}else {const existing=HTML.childNodes[1].childNodes;existing.push.apply(existing,body.after);}}return root}
  ...
  const{keys}=Object;function toString(input,config={}){const oldTree=createTree();const activeTasks=new Set(config.tasks||defaultTasks);activeTasks.delete(tasks.patchNode);activeTasks.delete(tasks.endAsTransaction);activeTasks.add(function endAsString(transaction){return serializeVTree(transaction.oldTree)});config.tasks=[...activeTasks];config.inner=true;let markup="";try{markup=Transaction.create(oldTree,input,config).start();}catch(e){release(oldTree);throw e}release(oldTree);return markup}function serializeAttributes(attributes){const attrs=keys(attributes);return attrs.length?" "+attrs.map(keyName=>{const value=attributes[keyName];const isFalsy=!value;const isDynamic=typeof value==="object"||typeof value==="function";if(value===true){return keyName}return `${keyName}${!isFalsy&&!isDynamic?`="${String(value)}"`:""}`}).join(" "):""}function serializeVTree(vTree){let output="";if(!vTree){return output}const{childNodes,nodeType,nodeName:tag,nodeValue,attributes}=vTree;if(nodeType===11){for(let i=0;i<childNodes.length;i++){output+=serializeVTree(childNodes[i]);}}else if(!childNodes.length&&nodeType===1){output+=`<${tag}${serializeAttributes(attributes)}></${tag}>`;}else if(nodeType===3){output+=nodeValue;}else if(childNodes.length){const children=childNodes.map(childNode=>`${serializeVTree(childNode)}`).join("");output+=`<${tag}${serializeAttributes(attributes)}>${children}</${tag}>`;}return output}

  const TOKEN="__DIFFHTML__";const{getOwnPropertyNames}=Object;const{isArray: isArray$1}=Array;const tokenEx=new RegExp(`${TOKEN}([^_]*)__`);const nextValue=(values)=>{const value=values.shift();return typeof value==="string"?escape(decodeEntities(value)):value};const interpolateAndFlatten=(childNode,supplemental)=>{let match=null;let newNodeName=childNode.rawNodeName;if(childNode.nodeType===NODE_TYPE.COMMENT){const parts=childNode.nodeValue.split(tokenEx);let newValue="";for(let i=0;i<parts.length;i++){const isDynamic=i%2!==0;if(isDynamic){newValue+=supplemental.attributes[parts[i]];}else {newValue+=parts[i];}}childNode.nodeValue=newValue;}if(match=tokenEx.exec(childNode.rawNodeName)){newNodeName=supplemental.tags[match[1]];childNode=createTree(newNodeName,childNode.attributes,childNode.childNodes);}for(const keyName of getOwnPropertyNames(childNode.attributes)){keyName.split(" ").forEach(keyName=>{const value=childNode.attributes[keyName];let newValue=value;let newKey=keyName;if(match=tokenEx.exec(value)){const parts=value.split(tokenEx);newValue="";if(parts.length===3&&parts[0]===EMPTY.STR&&parts[2]===EMPTY.STR){newValue=supplemental.attributes[parts[1]];}else {for(let i=0;i<parts.length;i++){const isDynamic=i%2!==0;if(isDynamic){newValue+=supplemental.attributes[parts[i]];}else {newValue+=parts[i];}}}}if(match=tokenEx.exec(keyName)){const parts=keyName.split(tokenEx);for(let i=0;i<parts.length;i++){if(i%2!==0){newKey=supplemental.attributes[parts[i]];}}}if(newKey){if(typeof newKey!=="string"){if(Array.isArray(newKey)){if(internalProcess.env.NODE_ENV!=="production"){throw new Error("Arrays cannot be spread as attributes")}delete childNode.attributes[keyName];}else {delete childNode.attributes[keyName];Object.assign(childNode.attributes,newKey);}}else {delete childNode.attributes[keyName];if(newKey==="childNodes"){childNode.childNodes.length=0;if(typeof newValue!=="string"){childNode.childNodes.push(createTree(newValue));}else {childNode.childNodes.push(createTree("#text",newValue));}}childNode.attributes[newKey]=newValue===undefined?true:newValue;}}if(childNode.attributes.key){childNode.key=childNode.attributes.key;}if(childNode.nodeName==="script"&&childNode.attributes.src){childNode.key=childNode.attributes.src;}});}if(match=tokenEx.exec(childNode.nodeValue)){const parts=childNode.nodeValue.split(tokenEx);const fragment=createTree();for(let i=0;i<parts.length;i++){if(i%2!==0){fragment.childNodes.push(createTree(supplemental.children[parts[i]]));}else if(parts[i]){fragment.childNodes.push(createTree("#text",parts[i]));}}return fragment}for(let i=0;i<childNode.childNodes.length;i++){const vTree=interpolateAndFlatten(childNode.childNodes[i],supplemental);if(vTree.nodeName==="#document-fragment"&&vTree.rawNodeName===vTree.nodeName){childNode.childNodes.splice(i,1,...vTree.childNodes);i-=1;}else {childNode.childNodes[i]=vTree;}}return childNode};function handleTaggedTemplate(strings,...values){const empty=createTree("#text",EMPTY.STR);if(!strings){return empty}else if(typeof strings==="string"){strings=[strings];}if(strings.length===1&&!values.length){if(!strings[0]){return empty}let{childNodes}=Internals.parse(strings[0]);const startNode=childNodes[0];const endNode=childNodes[childNodes.length-1];const isStartText=startNode.nodeType===NODE_TYPE.TEXT;const isEndText=endNode.nodeType===NODE_TYPE.TEXT;if(isStartText||isEndText){const trimmedNodes=[].concat(...childNodes);for(let i=0;i<trimmedNodes.length;i++){const node=trimmedNodes[i];if((node===startNode||node===endNode)&&node.nodeType===NODE_TYPE.TEXT&&!node.nodeValue.trim()){trimmedNodes.splice(i,1);}}if(trimmedNodes.length===1){childNodes=trimmedNodes;}}return createTree(childNodes.length===1?childNodes[0]:childNodes)}let HTML=EMPTY.STR;const supplemental={attributes:{},children:{},tags:{}};strings.forEach((string,i)=>{HTML+=string;if(values.length){const value=nextValue(values);const lastCharacter=HTML.trim().slice(-1);const lastTwoCharacters=HTML.trim().slice(-2);const isAttribute=HTML.lastIndexOf(">")<HTML.lastIndexOf("<");const isTag=Boolean(lastCharacter==="<"||lastTwoCharacters==="</");const isObject=typeof value==="object";const token=`${TOKEN}${i}__`;if(isTag){supplemental.tags[i]=value;HTML+=token;}else if(isAttribute){supplemental.attributes[i]=value;HTML+=token;}else if(isArray$1(value)||isObject){supplemental.children[i]=createTree(value);HTML+=token;}else if(value){HTML+=value;}}});const{childNodes}=Internals.parse(HTML);const vTree=interpolateAndFlatten(createTree(childNodes),supplemental);if(vTree.nodeType===NODE_TYPE.FRAGMENT&&vTree.childNodes.length===1){return vTree.childNodes[0]}return vTree}

  const{isArray}=Array;function use(middleware){const isFunction=typeof middleware==="function";const isObject=typeof middleware==="object";if(internalProcess.env.NODE_ENV!=="production"){if(!middleware||!isFunction&&!isObject||isArray(middleware)){throw new Error("Middleware must be a function or plain object")}}const{subscribe,unsubscribe,createTreeHook,createNodeHook,syncTreeHook,releaseHook,parseHook}=middleware;isFunction&&MiddlewareCache.add(middleware);subscribe&&subscribe(Internals);createTreeHook&&CreateTreeHookCache.add(createTreeHook);createNodeHook&&CreateNodeHookCache.add(createNodeHook);syncTreeHook&&SyncTreeHookCache.add(syncTreeHook);releaseHook&&ReleaseHookCache.add(releaseHook);parseHook&&ParseHookCache.add(parseHook);return ()=>{isFunction&&MiddlewareCache.delete(middleware);unsubscribe&&unsubscribe(Internals);createTreeHook&&CreateTreeHookCache.delete(createTreeHook);createNodeHook&&CreateNodeHookCache.delete(createNodeHook);syncTreeHook&&SyncTreeHookCache.delete(syncTreeHook);releaseHook&&ReleaseHookCache.delete(releaseHook);parseHook&&ParseHookCache.delete(parseHook);}}

  const __VERSION__="1.0.0-beta.30";

  defaultTasks.splice(defaultTasks.indexOf(reconcileTrees),0,parseNewTree);Internals["VERSION"]=__VERSION__;Internals["parse"]=parse;const api={};api["VERSION"]=__VERSION__;api["release"]=release;api["createTree"]=createTree;api["use"]=use;api["outerHTML"]=outerHTML;api["innerHTML"]=innerHTML;api["toString"]=toString;api["html"]=handleTaggedTemplate;api["Internals"]=Internals;if($$diffHTML in globalThis){const existingApi=globalThis[$$diffHTML];if(__VERSION__!==existingApi["VERSION"]){console.log(`Loaded ${__VERSION__} after ${existingApi.VERSION}`);}}globalThis[$$diffHTML]=api;if(globalThis.devTools){globalThis.unsubscribeDevTools=use(globalThis.devTools(Internals));}
