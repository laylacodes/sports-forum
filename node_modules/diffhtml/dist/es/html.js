 ...
 */import createTree from"./tree/create.js";import Internals from"./util/internals.js";import escape from"./util/escape.js";import decodeEntities from"./util/decode-entities.js";import internalProcess from"./util/process.js";import{EMPTY,NODE_TYPE}from"./util/types.js";// Magic token used for interpolation.
...
// it is escaped.
const nextValue=(/** @type {any[]} */values)=>{const value=values.shift();return typeof value==="string"?escape(decodeEntities(value)):value};/**
...
if(childNode.nodeType===NODE_TYPE.COMMENT){const parts=childNode.nodeValue.split(tokenEx);let newValue="";for(let i=0;i<parts.length;i++){const isDynamic=i%2!==0;if(isDynamic){newValue+=supplemental.attributes[parts[i]]}else{newValue+=parts[i]}}childNode.nodeValue=newValue}// Node name
if(match=tokenEx.exec(childNode.rawNodeName)){newNodeName=supplemental.tags[match[1]];childNode=createTree(newNodeName,childNode.attributes,childNode.childNodes)}// Attributes
...
if(parts.length===3&&parts[0]===EMPTY.STR&&parts[2]===EMPTY.STR){newValue=supplemental.attributes[parts[1]]}else{for(let i=0;i<parts.length;i++){const isDynamic=i%2!==0;if(isDynamic){newValue+=supplemental.attributes[parts[i]]}else{newValue+=parts[i]}}}}// Check for dynamic key and assign to newKey.
if(match=tokenEx.exec(keyName)){const parts=keyName.split(tokenEx);for(let i=0;i<parts.length;i++){if(i%2!==0){newKey=supplemental.attributes[parts[i]]}}}if(newKey){if(typeof newKey!=="string"){if(Array.isArray(newKey)){if(internalProcess.env.NODE_ENV!=="production"){throw new Error("Arrays cannot be spread as attributes")}delete childNode.attributes[keyName]}else{delete childNode.attributes[keyName];Object.assign(childNode.attributes,newKey)}}else{delete childNode.attributes[keyName];if(newKey==="childNodes"){childNode.childNodes.length=0;if(typeof newValue!=="string"){childNode.childNodes.push(createTree(newValue))}else{childNode.childNodes.push(createTree("#text",newValue))}}childNode.attributes[newKey]=newValue===undefined?true:newValue}}if(childNode.attributes.key){childNode.key=childNode.attributes.key}if(childNode.nodeName==="script"&&childNode.attributes.src){childNode.key=childNode.attributes.src}})}// Node value
if(match=tokenEx.exec(childNode.nodeValue)){const parts=childNode.nodeValue.split(tokenEx);const fragment=createTree();for(let i=0;i<parts.length;i++){if(i%2!==0){fragment.childNodes.push(createTree(supplemental.children[parts[i]]))}else if(parts[i]){fragment.childNodes.push(createTree("#text",parts[i]))}}return fragment}// Children
 ...
 */export default function handleTaggedTemplate(strings,...values){const empty=createTree("#text",EMPTY.STR);// Do not attempt to parse empty strings.
...
else if(isArray(value)||isObject){supplemental.children[i]=createTree(value);HTML+=token}// Injected as something else in the markup or undefined, ignore
...
const{childNodes}=Internals.parse(HTML);// Pass through flatten and interpolate dynamic data.
const vTree=interpolateAndFlatten(createTree(childNodes),supplemental);// Loop through all nodes and apply the dynamic attributes and flatten
// fragments.
if(vTree.nodeType===NODE_TYPE.FRAGMENT&&vTree.childNodes.length===1){return vTree.childNodes[0]}return vTree}