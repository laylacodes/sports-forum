 ...
 */import{StateCache,NODE_TYPE}from"../util/types.js";import{protectVTree}from"../util/memory.js";import createTree from"../tree/create.js";import release from"../release.js";/**
 ...
 */export default function reconcileTrees(transaction){const{state,mount,input,config:options}=transaction;const{inner}=options;const mountAsHTMLEl=/** @type {HTMLElement} */mount;// Look if any changes happened before the async mutation callback.
...
// slate.
release(mountAsHTMLEl);// Ensure the mutation observer is reconnected.
if(mountAsHTMLEl.ownerDocument&&state.mutationObserver){state.mutationObserver.observe(mountAsHTMLEl,{subtree:true,childList:true,attributes:true,characterData:true})}state.oldTree=createTree(mountAsHTMLEl);protectVTree(state.oldTree);StateCache.set(mount,state)}const{nodeName,attributes}=state.oldTree;// If we are in a render transaction where no markup was previously parsed
// then reconcile trees will attempt to create a tree based on the incoming
// markup (JSX/html/etc).
if(!transaction.newTree){transaction.newTree=createTree(input)}const inputAsVTree=/** @type {VTree} */transaction.newTree;// When `inner === false` this means we are doing outerHTML operation.  The
...
if(!inner&&inputAsVTree.nodeType===NODE_TYPE.FRAGMENT&&// Do not modify the new children when comparing two fragments.
state.oldTree.nodeType!==NODE_TYPE.FRAGMENT){/** @type {VTree[]} */let foundElements=[];for(let i=0;i<inputAsVTree.childNodes.length;i++){const value=inputAsVTree.childNodes[i];const isText=value.nodeType===NODE_TYPE.TEXT;// This is most likely the element that is requested to compare to. Will
...
else if(foundElements.length>1){transaction.newTree=createTree(inputAsVTree.childNodes)}}// Associate the old tree with this brand new transaction.
...
if(inner&&oldTree&&newTree){const isUnknown=typeof newTree.rawNodeName!=="string";const isFragment=newTree.nodeType===NODE_TYPE.FRAGMENT;const children=isFragment&&!isUnknown?newTree.childNodes:newTree;transaction.newTree=createTree(nodeName,attributes,children)}}