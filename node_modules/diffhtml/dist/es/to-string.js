 ...
 */import createTree from"./tree/create.js";import Transaction,{defaultTasks,tasks}from"./transaction.js";import release from"./release.js";const{keys}=Object;/**
 ...
 * @return {string}
 */export default function toString(input,config={}){const oldTree=createTree();const activeTasks=new Set(config.tasks||defaultTasks);activeTasks.delete(tasks.patchNode);// Replace the `endAsTransaction` task with the string return value.
activeTasks.delete(tasks.endAsTransaction);activeTasks.add(function endAsString(/** @type {Transaction} */transaction){return serializeVTree(transaction.oldTree)});config.tasks=[...activeTasks];config.inner=true;let markup="";try{markup=/** @type {string} */Transaction.create(oldTree,input,config).start()}catch(e){release(oldTree);throw e}release(oldTree);return markup}/**
 * serializeAttributes
 *
...
else if(childNodes.length){const children=childNodes.map(childNode=>`${serializeVTree(childNode)}`).join("");output+=`<${tag}${serializeAttributes(attributes)}>${children}</${tag}>`}return output}