 ...
 */import internalProcess from"./process.js";import globalThis from"./global.js";const{parseInt}=Number;const{parse}=JSON;/** @type {Config} */export const globalConfig={collectMetrics:true,executeScripts:true};/**
 ...
 *
 * @return {unknown}
 */function formatValue(value,type){const valueAsString=String(value);switch(type){case"boolean":{return valueAsString!=="false"}case"string":{return valueAsString}case"number":{return parseInt(valueAsString,10)}case"object":{return parse(valueAsString)}}}/**
 ...
 *
 * @return {unknown}
 */export default function getConfig(name,defaultValue,type=typeof defaultValue,overrides){const{location,URLSearchParams}=globalThis;const hasSearchParams=typeof URLSearchParams!=="undefined";const hasLocation=typeof location!=="undefined";const useSearchParams=hasSearchParams&&hasLocation;const useEnv=internalProcess.env;// Allow bypassing any lookups if overrides are passed and match the config
// being looked up.
...
const upperKey=keyName.toUpperCase();if(useEnv&&upperKey in internalProcess.env){return formatValue(internalProcess.env[upperKey.toUpperCase()],type)}return defaultValue}