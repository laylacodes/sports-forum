 ...
 */import createTree from"../tree/create.js";import getConfig from"./config.js";import{NODE_TYPE,EMPTY}from"./types.js";const rawElementsDefaults=["script","noscript","style","template"];/**
 *
 ...
 * @type {any}
 */const endTagOmissionRules={li:{li:EMPTY.NUM},dt:{dt:EMPTY.NUM,dd:EMPTY.NUM},dd:{dt:EMPTY.NUM,dd:EMPTY.NUM},td:{td:EMPTY.NUM,th:EMPTY.NUM},th:{td:EMPTY.NUM,th:EMPTY.NUM},tbody:{tbody:EMPTY.NUM,tfoot:EMPTY.NUM},tfoot:{tbody:EMPTY.NUM,tfoot:EMPTY.NUM}};// List of regular expressions to match various HTML features.
export const openComment=/<!--/g;export const closeComment=/-->/g;// Extract the tagName from an opening tag. There must not be any whitespace
// between the opening angle bracket and the word/namespace.
export const openTag=/<([^\s\\/>]*)\s?/g;export const closeTag=/\s?(\/>)|(<\/(.*?)>)/g;// Find all values within quotes and up to `>`. Support interpolated values
// with __DIFFHTML_TOKEN__ format.
export const attribute=/([^>\/\n ])*?(["'])(?:(?=(\\?))\3.)*?\2|.*?(?=\/>|>|\n| )/gsm;export const parseAttr=/([^=]*)=(.*)|([^>])/gsm;// Text nodes are anything that isn't <.
export const text=/([^<]*)/g;/**
 ...
 */export default function parse(html,options={}){// Always start with a fragment container when parsing.
const root=createTree("#document-fragment",null,[]);// If no markup is provided, return an empty text node. This is a fast path
// to circumvent extra work in this case.
if(!html){root.childNodes.push(createTree("#text",EMPTY.STR));return root}// If there are no parser configuration options passed, use an empty object.
if(!options.parser){/** @type {ParserConfig} */options.parser=EMPTY.OBJ}// Elements that have all nested children converted into text, like script
// and style tags.
const blockText=new Set(/** @type {string[]} */getConfig("rawElements",rawElementsDefaults,"array",options.parser));// Elements that are automatically self closed, and never contain children.
const voidElements=new Set(/** @type {string[]} */getConfig("voidElements",voidElementsDefaults,"array",options.parser));// If there are no brackets, we can avoid some extra work by treating this as
// text. This is a fast path for text.
if(!html.includes("<")&&!html.includes(">")){const newTree=createTree("#text",html);root.childNodes.push(newTree);return root}// Contains the active hierarchy, its length will be that of the deepest
// element crawled.
...
const newTree=createTree(stack.pop());// Reset the pointer to the parent.
...
index:openCommentIndex}=shouldSeekComment&&openComment.exec(html)||EMPTY.OBJ;// There are no remaining comments, so skip this check. This is very
...
let{index:closeCommentIndex}=closeComment.exec(html)||EMPTY.OBJ;// Default to the end of the markup if no end comment is found.
if(closeCommentIndex===-1){closeCommentIndex=html.length}const comment=createTree("#comment");comment.nodeValue=html.slice(i+4,closeCommentIndex);pointer.childNodes.push(comment);i=closeCommentIndex+3;continue}// Open tags.
const{0:fullOpenTagMatch,1:tagName,index:openTagIndex}=openTag.exec(html)||EMPTY.OBJ;// Only open a tag if it contains a tag name.
...
const newTree={rawNodeName:tagName,nodeName:tagName,childNodes:[],attributes:{},nodeType:EMPTY.NUM,nodeValue:EMPTY.STR,key:EMPTY.STR};const supportsEndTagOmission=endTagOmissionRules[tagName];// We can't nested a div inside a p, we can't nest an li inside an li
...
const{0:fullAttributeMatch,index:attributeIndex}=attribute.exec(html)||EMPTY.OBJ;const attributeMatchTrim=attributeIndex===i&&fullAttributeMatch.trim();if(isOpen&&attributeIndex===i){// Skip whitespace
...
if(isBlockElement){const closeTag=`</${pointer.nodeName}>`;let closeTagIndex=html.indexOf(closeTag,i);if(closeTagIndex===-1){closeTagIndex=html.length}const innerText=html.slice(i,closeTagIndex);if(innerText){pointer.childNodes.push(createTree("#text",innerText))}i=closeTagIndex+closeTag.length;isOpen=false;resetPointer();continue}// Close opened tags.
...
const{0:fullCloseTagMatch,3:closeTagName,index:closeTagIndex}=closeTag.exec(html)||EMPTY.OBJ;// Look for closing tags
...
const{0:fullTextMatch,index:textIndex}=text.exec(html)||EMPTY.OBJ;if(!isOpen&&textIndex===i&&fullTextMatch.length){const newTree=createTree("#text",fullTextMatch);pointer.childNodes.push(newTree);i=textIndex+fullTextMatch.length;if(i===html.length&&isNotRoot){resetPointer()}continue}// Use remaining values as text
pointer.childNodes.push(createTree("#text",html.slice(i,html.length)));i=html.length}// This is an entire document, so only allow the HTML children to be
// body or head.
...
else if(el.nodeType===NODE_TYPE.ELEMENT){if(beforeHead&&beforeBody)head.before.push(el);else if(!beforeHead&&beforeBody)head.after.push(el);else if(!beforeBody)body.after.push(el)}});// Ensure the first element is the HEAD tag.
if(!HTML.childNodes[0]||HTML.childNodes[0].nodeName!=="head"){const headInstance=createTree("head",null,[]);if(headInstance){const existing=headInstance.childNodes;existing.unshift.apply(existing,head.before);existing.push.apply(existing,head.after);HTML.childNodes.unshift(headInstance)}}else{const existing=HTML.childNodes[0].childNodes;existing.unshift.apply(existing,head.before);existing.push.apply(existing,head.after)}// Ensure the second element is the body tag.
if(!HTML.childNodes[1]||HTML.childNodes[1].nodeName!=="body"){const bodyInstance=createTree("body",null,[]);if(bodyInstance){const existing=bodyInstance.childNodes;existing.push.apply(existing,body.after);HTML.childNodes.push(bodyInstance)}}else{const existing=HTML.childNodes[1].childNodes;existing.push.apply(existing,body.after)}}return root}