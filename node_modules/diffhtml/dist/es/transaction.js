 ...
 */import{MiddlewareCache,StateCache,NodeCache,EMPTY}from"./util/types.js";import makeMeasure from"./util/make-measure.js";import internalProcess from"./util/process.js";import{protectVTree}from"./util/memory.js";import globalThis from"./util/global.js";import shouldUpdate from"./tasks/should-update.js";import reconcileTrees from"./tasks/reconcile-trees.js";import syncTrees from"./tasks/sync-trees.js";import patchNode from"./tasks/patch-node.js";import endAsTransaction from"./tasks/end-as-transaction.js";import release from"./release.js";import getConfig from"./util/config.js";import hasModule from"./util/has-module.js";const{assign}=Object;export const defaultTasks=[shouldUpdate,reconcileTrees,syncTrees,patchNode,endAsTransaction];export const tasks={shouldUpdate,reconcileTrees,syncTrees,patchNode,endAsTransaction};export default class Transaction{/**
...
if(retVal!==undefined&&retVal!==transaction){return retVal}}return retVal}/**
   *
   * @param {Transaction} transaction
   */static assert(transaction){if(internalProcess.env.NODE_ENV!=="production"){if(typeof transaction.mount!=="object"||!transaction.mount){throw new Error("Transaction requires a DOM Node mount point")}if(transaction.aborted&&transaction.completed){throw new Error("Transaction was previously aborted")}if(transaction.completed){throw new Error("Transaction was previously completed")}}}/**
   *
   * @param {Transaction} transaction
   */static invokeMiddleware(transaction){const{state:{measure},tasks}=transaction;MiddlewareCache.forEach(fn=>{const label=`invoke ${fn.name||"anon"}`;measure(label);// Invoke all the middleware passing along this transaction as the only
...
const result=fn(transaction);if(result){tasks.push(result)}measure(label)})}/**
   ...
   */constructor(mount,input,config){_defineProperty(this,"state",EMPTY.OBJ);_defineProperty(this,"mount",EMPTY.OBJ);_defineProperty(this,"input",EMPTY.OBJ);_defineProperty(this,"oldTree",undefined);_defineProperty(this,"newTree",undefined);_defineProperty(this,"tasks",[]);_defineProperty(this,"patches",[]);this.mount=mount;this.input=input;this.config=config;const useObserver=!config.disableMutationObserver&&"MutationObserver"in(globalThis.window||EMPTY.OBJ);this.state=StateCache.get(mount)||/** @type {TransactionState} */{measure:makeMeasure(this),svgElements:new Set,scriptsToExecute:new Map,mutationObserver:useObserver&&new globalThis.window.MutationObserver(()=>{this.state.isDirty=true})};this.tasks=/** @type {Function[]} */getConfig("tasks",defaultTasks,undefined,config).slice();// Store calls to trigger after the transaction has ended.
this.endedCallbacks=new Set;StateCache.set(mount,this.state)}/**
   * @return {Transaction | unknown}
   */start(){if(internalProcess.env.NODE_ENV!=="production"){Transaction.assert(this)}const{state:{measure},tasks}=this;const takeLastTask=tasks.pop();// Start measuring a render for performance tracing.
...
StateCache.forEach(state=>{if(state.mutationObserver){state.mutationObserver.takeRecords()}});// If MutationObserver is available, look for changes.
...
// position.
scriptsToExecute.forEach((type,vTree)=>{const oldNode=NodeCache.get(vTree);// Reset the type attribute back to the original.
oldNode.type=type;if(!config.executeScripts||hasModule()&&type==="nomodule"){return}// Copy over properties to the new script element.
...
// re-assign.
if(StateCache.has(oldNode)){release(oldNode);StateCache.set(newNode,state)}// Replace the node association.
NodeCache.set(vTree,newNode);// Replace the scripts to trigger default browser behavior.
...
if(state.oldTree)protectVTree(state.oldTree);return this}/**
   * @param {Function} callback
   */onceEnded(callback){this.endedCallbacks.add(callback)}/** @type {TransactionState} */}